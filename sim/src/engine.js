 ...
 *   Insert: printable, Enter, Backspace, Escape
 *   Replace: R (overtype)
 *   Command: :w :q :wq :q! :42
 */
...
export class VimEngine {
  constructor({ rows = 20, cols = 40, lines = [''] } = {}) {
    ...
    this._lastMacroRegister = '';  // for @@ (replay last)
  ...
  _normalKey(key) {
    ...
    if (this._pendingF) {
      ...
      if (key.length === 1 && key !== 'Escape') {
        ...
        if (this._pendingOp) {
          ...
          this._desiredCol = this.cursor.col;
        }
      } else {
        this._pendingOp = '';
        this._pendingCount = '';
    ...
    switch (key) {
      ...
      case 'j': case 'ArrowDown': {
        ...
        this.cursor.col = Math.min(this._desiredCol, this._maxCol());
        break;
      }
      case 'k': case 'ArrowUp': {
        const count = this._getCount();
      ...
      // Word motions
      case 'w': { const c = this._getCount(); for (let i = 0; i < c; i++) this._moveWordForward(false); this._desiredCol = this.cursor.col; break; }
      case 'W': { const c = this._getCount(); for (let i = 0; i < c; i++) this._moveWordForward(true); this._desiredCol = this.cursor.col; break; }
      case 'b': { const c = this._getCount(); for (let i = 0; i < c; i++) this._moveWordBackward(false); this._desiredCol = this.cursor.col; break; }
      case 'B': { const c = this._getCount(); for (let i = 0; i < c; i++) this._moveWordBackward(true); this._desiredCol = this.cursor.col; break; }
      case 'e': { const c = this._getCount(); for (let i = 0; i < c; i++) this._moveWordEnd(false); this._desiredCol = this.cursor.col; break; }
      case 'E': { const c = this._getCount(); for (let i = 0; i < c; i++) this._moveWordEnd(true); this._desiredCol = this.cursor.col; break; }

      // Line motions
      case '0': this.cursor.col = 0; this._desiredCol = 0; break;
      case '^': this.cursor.col = this._firstNonBlank(this.cursor.row); this._desiredCol = this.cursor.col; break;
      case '$': {
        const count = this._getCount();
      ...
      // Paragraph
      case '{': { const c = this._getCount(); const saved = { row: this.cursor.row, col: this.cursor.col }; let ok = true; for (let i = 0; i < c; i++) { if (!this._moveParagraphBackward()) { ok = false; break; } } if (!ok) { this.cursor.row = saved.row; this.cursor.col = saved.col; } this._desiredCol = this.cursor.col; break; }
      case '}': { const c = this._getCount(); const saved = { row: this.cursor.row, col: this.cursor.col }; let ok = true; for (let i = 0; i < c; i++) { if (!this._moveParagraphForward()) { ok = false; break; } } if (!ok) { this.cursor.row = saved.row; this.cursor.col = saved.col; } this._desiredCol = this.cursor.col; break; }
      ...
      case '|': {
        const c = this._getCount() - 1;
        this.cursor.col = Math.min(c, this._maxCol());
        this._desiredCol = this.cursor.col;
        break;
      }
      ...
      case '?':
        ...
        this.commandLine = '?';
        return;
      case 'n': { const c = this._getCount(); for (let i = 0; i < c; i++) this._searchNext(this._searchForward); this._showCurSearch = true; break; }
      case 'N': { const c = this._getCount(); for (let i = 0; i < c; i++) this._searchNext(!this._searchForward); this._showCurSearch = true; break; }
      case '*': {
        const w = this._wordUnderCursor();
        if (w) { this._searchPattern = '\\b' + w + '\\b'; this._searchForward = true; this._searchNext(true); this._showCurSearch = true; }
        break;
      }
      case '#': {
        const w = this._wordUnderCursor();
        if (w) { this._searchPattern = '\\b' + w + '\\b'; this._searchForward = false; this._searchNext(false); this._showCurSearch = true; }
        break;
      }
  ...
  _handleG(key) {
    switch (key) {
      ...
      case 'v': break; // gv — reselect (not implemented)
      case 'i':
  ...
  _executeCommand() {
    ...
    // : commands — file/quit operations
    // Set _lastExCommand so SessionManager can intercept; clear commandLine.
    if (/^(w|wq|x|q|q!)(\s|$)/.test(cmd) || /^e(\s|$)/.test(cmd) || cmd.startsWith('!')) {
  ...
  _visualKey(key) {
    ...
    if (key === 'v') {
      if (this.mode === Mode.VISUAL) { this.mode = Mode.NORMAL; this.commandLine = ''; }
      else { this.mode = Mode.VISUAL; this.commandLine = '-- VISUAL --'; }
      this._pendingCount = ''; return;
    }
    if (key === 'V') {
      if (this.mode === Mode.VISUAL_LINE) { this.mode = Mode.NORMAL; this.commandLine = ''; }
    ...
    // Actions
    const actionKeys = new Set(['d','c','y','>','<','~','U','u','J','p','x','s','r']);
  ...
  _updateCurSearchPos() {
    ...
    try {
      const re = new RegExp(this._searchPattern, 'i');
      const line = this.buffer.lines[this.cursor.row] || '';
  ...
  _executeOperatorRange(range) {
    ...
    switch (op) {
      case 'd': {
        ...
        this.cursor.col = Math.min(startCol, this._maxCol());
        this._startRecording(); this._stopRecording();
        this._redoStack = [];
        break;
      }
  ...
  _doLineOperation(op, count) {
    ...
    if (op !== '>' && op !== '<') this._desiredCol = this.cursor.col;
    this._pendingOp = '';
  }
  ...
  _searchNext(forward) {
    ...
    let pattern;
    try { pattern = new RegExp(this._searchPattern, 'i'); } catch { return false; }
    const sr = this.cursor.row, sc = this.cursor.col;
    const lc = this.buffer.lineCount;
    if (forward) {
