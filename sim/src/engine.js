...
export class VimEngine {
  constructor({ rows = 20, cols = 40, lines = [''] } = {}) {
    ...
    this._pendingPromptCarry = null; // message preserved when `:` re-enters cmdline from prompt
    this._recentMessageLines = null; // sticky cmdline message lines (for promotion)
    this._recentFromEx = false;      // whether the recent sticky was set by an ex command
  ...
  feedKey(key) {
    ...
    // If in message prompt mode, most keys dismiss it.
    // - `:` enters cmdline mode while preserving the message (so it can
    //   accumulate with a subsequent message), matching nvim's
    //   hit-enter-prompt behaviour.
    // - <CR>, <Space>, <Esc>, q, Ctrl-C, Ctrl-Y are consumed by the prompt
    //   itself (acknowledge and discard).
    // - Other keys dismiss the prompt and then the key is processed
    //   normally (per :help hit-enter).
    if (this._messagePrompt) {
      const consumeKey = (key === 'Enter' || key === 'Ctrl-J' || key === 'Ctrl-M'
                       || key === 'Space' || key === ' '
                       || key === 'Escape' || key === 'Esc'
                       || key === 'q' || key === 'Ctrl-C' || key === 'Ctrl-Y');
      if (key === ':') {
        this._pendingPromptCarry = this._messagePrompt;
        this._messagePrompt = null;
        // Fall through to normal handling so `:` enters COMMAND mode
      } else if (consumeKey) {
        this._messagePrompt = null;
        this.commandLine = '';
        this._updateStatus();
        ...
        // Dismiss prompt; fall through and process the key normally
        this._messagePrompt = null;
        this.commandLine = '';
      }
    }
  ...
  _handleZ(key) {
    ...
    switch (key) {
      ...
      case 'x':
      case 'X': {
        // zx/zX — recompute folds and open/close as appropriate
        if (this._folds.length === 0) {
          this._reportCmdMessage({ error: 'E490: No fold found' });
        } else if (key === 'X') {
          for (const f of this._folds) f.closed = true;
        }
  ...
  _reportCmdMessage(mp, fromEx = false) {
    if (this._pendingPromptCarry) {
      ...
      // New error clobbers any prior carry (matches nvim: second :b foo wipes
      // the prior E86). New info/warn accumulates atop the prior message.
      if (mp.error) {
        this._messagePrompt = mp;
      } else {
        const prevLines = this._promptToLines(prev);
        const next = this._promptToLines(mp);
        this._messagePrompt = { lines: [...prevLines, ...next] };
      }
      this.commandLine = '';
      this._recentMessageLines = null;
      this._recentFromEx = false;
    ...
    if (this._recentMessageLines && this._recentMessageLines.length > 0
        && (this._recentFromEx || fromEx)) {
      // A prior sticky is pending and at least one party is ex-driven —
      // second message elevates to Press-ENTER (matches nvim accumulation
      // semantics for ex-command output). Two consecutive purely
      // normal-mode errors do NOT elevate (each one replaces the cmdline).
      if (mp.error) {
        // Error clobbers prior message
        this._messagePrompt = mp;
      } else {
        // Info/warn accumulates with the prior message
        const next = this._promptToLines(mp);
        this._messagePrompt = { lines: [...this._recentMessageLines, ...next] };
      }
      this._recentMessageLines = null;
      this._recentFromEx = false;
      this.commandLine = '';
  ...
  _buildPromptLines(items) {
    // Build {text, runs} entries with named fg from theme via screen.js conventions.
    // Theme colors hardcoded here for the small fixed set we use.
    const colorMap = {
      errorFg: 'e95678',
      cmdFg:   'd4d4d4',
      promptFg:'e6db74',
    ...
    const cols = this.cols;
    return items.map(it => {
      const fg = colorMap[it.fg] || colorMap.cmdFg;
      const text = it.text || '';
      const truncated = text.length > cols ? text.slice(0, cols) : text;
      const lineText = truncated + ' '.repeat(Math.max(0, cols - truncated.length));
      const runs = [];
      if (truncated.length > 0) {
        const r = { n: truncated.length, fg, bg: cmdBg };
        if (it.b) r.b = true;
        runs.push(r);
      }
      if (truncated.length < cols) runs.push({ n: cols - truncated.length, fg: colorMap.cmdFg, bg: cmdBg });
      return { text: lineText, runs };
    });
  }

  /**
   * Lower-level helper: convert any `_messagePrompt` shape (info/warn/error/lines)
   * into the structured `lines` form so it can be accumulated with subsequent
   * messages. Returns an array of `{text, runs}` entries.
   */
  _promptToLines(mp) {
    if (!mp) return [];
    if (mp.lines) return mp.lines;
    const fgName = mp.error ? 'errorFg' : mp.warn ? 'promptFg' : 'cmdFg';
    const isBold = !!mp.error;
    const raw = mp.error || mp.warn || mp.info || '';
    return this._buildPromptLines(
      raw.split('\n').map(t => ({ text: t, fg: fgName, b: isBold }))
    );
  }

  /**
   * Show a new message prompt, accumulating with any pending carry from
   * a prior prompt that the user dismissed by typing `:` (nvim's
   * hit-enter-prompt → cmdline-mode transition keeps the prior message
   * on screen).
   */
  _setPromptWithCarry(newMp) {
    if (this._pendingPromptCarry) {
      const prev = this._promptToLines(this._pendingPromptCarry);
      const next = this._promptToLines(newMp);
      this._pendingPromptCarry = null;
      this._messagePrompt = { lines: [...prev, ...next] };
    } else {
      this._messagePrompt = newMp;
    }
  }
  ...
  _handleCtrlW(key) {
    switch (key) {
      ...
      case 'x':
        ...
        break;
      case 'c': case 'q':
        // Close current window (internally invokes :close — treat as ex)
        this._doCloseWindow(true);
        break;
      case 'o':
        // Only: close all other windows (internally invokes :only — treat as ex)
  ...
  _handleBracketCommand(bracket, key) {
    ...
    switch (combo) {
      ...
      case '[d': case ']d': case '[D': case ']D':
        // Diagnostic navigation — sim has no LSP diagnostics, always "no more"
        this._messagePrompt = { warn: 'No more valid diagnostics to move to' };
        break;
      case '[f': case ']f':
