    ...
    while i < n:
        ...
        if c == '"':
            if not prev or prev.isspace() or prev in "([{<-/\u2013\u2014":
                ...
                # happens to sit after a space (often after markup like
                # a code or key span whose last_char was set to " ", e.g.
                # `register `a`"; the full ...`).
    ...
    for m in _combined.finditer(text):
        ...
        if kind == "key":
            ...
            if (len(k) >= 3 and k.startswith(":")
                ...
                pill = f"\\code{{{tex_escape_inline_code(k)}}}{idx}"
            elif plain_keys:
                pill = f"\\texttt{{{tex_escape_inline_code(k)}}}{idx}"
            else:
                pill = f"{_split_key_for_wrap(k)}{idx}"
            # Glue trailing punctuation to the pill so a line break can't
            # separate them. \key{} ends with an internal \allowbreak,
            # which would otherwise let "{key:D}, on the other hand" wrap
            # the comma alone onto the next line, or strand a closing
            # bracket like ")" after "{key:*})" on its own line. For
            # closing brackets we wrap pill+bracket in \mbox{} (penalty
            # \nobreak isn't enough to defeat the trailing \allowbreak
            # inside \key{} for the bracket case in tabularx X cells).
            nxt = text[m.end():m.end() + 1]
            if nxt in ")]}":
                out.append(f"\\mbox{{{pill}{tex_escape(nxt)}}}")
                pos = m.end() + 1
                last_char = nxt
                continue
            out.append(pill)
...
def render_block(b, *, index, examples) -> list[str]:
    ...
    if bt == "heading":
        ...
        if headers:
            ...
            table_lines.append("\\arrayrulecolor{rowrule}")
            for ridx, row in enumerate(rows):
                ...
                # Faint hairline between data rows (skip after the last
                # row so the table doesn't end with a stray bottom rule).
                if ridx < len(rows) - 1:
                    table_lines.append("\\hline")
            table_lines.append("\\arrayrulecolor{black}")
...
\usepackage{colortbl}  % \arrayrulecolor for faint inter-row rules
\definecolor{rowrule}{gray}{0.85}  % faint hairline between table rows
