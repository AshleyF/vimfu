...
% follow" mechanism. URL drops to \scriptsize: it's only a fallback for
% readers who can't scan, and the smaller size keeps the callout calm.
%
% The right column uses [t]-alignment + an explicit top pad so the TITLE
% line lands at the QR's vertical center (and the URL hangs just below),
% rather than [c]-aligning the whole title+URL block. This keeps the
% bold title visually centered on the QR — the URL reads as a quiet
% fall-back below.
\newcommand{\qrcallout}[3]{%
  \par\medskip\noindent\begin{minipage}[c]{0.12\linewidth}%
    \includegraphics[width=\linewidth]{#1}%
  \end{minipage}\hfill\begin{minipage}[t]{0.84\linewidth}%
    \vspace*{\dimexpr 0.06\linewidth - 0.7\baselineskip\relax}%
    {\sffamily\bfseries #2}\par\smallskip{\scriptsize\sffamily #3}%
...
\newcommand{\qrcalloutpair}[6]{%
  ...
  \begin{minipage}[t]{0.48\linewidth}%
    ...
    \end{minipage}\hfill\begin{minipage}[t]{0.71\linewidth}%
      \vspace*{\dimexpr 0.12\linewidth - 0.7\baselineskip\relax}%
