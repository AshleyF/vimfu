...
def _parse_qrcallout(s: str, start: int):
    """Parse ``\\qrcallout{a}{b}{c}`` at position ``start``.

    Returns ((a, b, c), end_index) where end_index is one past the final
    ...
    for _ in range(3):
        ...
        try:
            ...
            return None
        args.append(arg)
    return (args[0], args[1], args[2]), pos
...
def render_block(b, *, index, examples) -> list[str]:
    ...
    if bt == "example":
        ...
        out.append(
            ...
            + "}{\\linkicon\\iconsep Try this in the simulator}{" + tex_escape(qr_url) + "}"
        )
    ...
    if bt == "heading":
        ...
        if isinstance(lesson, int) or (isinstance(lesson, str) and str(lesson).strip().isdigit()):
            ...
            out.append(
                "\\qrcallout{" + latex_path(qr_p)
                + "}{\\playicon\\iconsep " + tex_escape(title) + "}{" + tex_escape(qr_url) + "}"
            )
        else:
        ...
        if slug:
            ...
            out.append(
                "\\qrcallout{" + latex_path(qr_p)
                + "}{\\linkicon\\iconsep " + cap + "}{" + tex_escape(qr_url) + "}"
            )
        return out
...
def render_topic_body(t, index, examples) -> str:
    ...
    if "tmux" in (t.get("__part_dir") or ""):
        ...
        out.append(
            "\\qrcallout{" + latex_path(qr_p)
            + "}{\\linkicon\\iconsep Spin up tmux in the simulator}{" + tex_escape(qr_url) + "}"
        )
    ...
    if vids:
        ...
        for v in vids:
            ...
            if v.get("published") and lesson_n is not None:
                ...
                out.append(
                    "\\qrcallout{" + latex_path(qr_p)
                    + "}{\\playicon\\iconsep " + title + "}{" + tex_escape(qr_url) + "}"
                )
            else:
...
\newcommand{\examplefigure}[1]{%
  \par\medskip\begin{center}%
    \fcolorbox{figureborder}{white}{\includegraphics[width=0.55\linewidth]{#1}}%
  \end{center}\par}
...
\newcommand{\photofigure}[4]{%
  \par\medskip\begin{center}%
    ...
    \fcolorbox{figureborder}{white}{\includegraphics[width=#2\linewidth]{#1}}}%
    \par\smallskip
    {\small\itshape #3}%
...
\newcommand{\qrcallout}[3]{%
  ...
  \end{minipage}\hfill\begin{minipage}[c]{0.84\linewidth}%
...
\newcommand{\qrcalloutpair}[6]{%
  \par\medskip\noindent
  \begin{minipage}[t]{0.48\linewidth}%
    \begin{minipage}[c]{0.25\linewidth}%
      \includegraphics[width=\linewidth]{#1}%
    \end{minipage}\hfill\begin{minipage}[c]{0.71\linewidth}%
      {\sffamily\bfseries\small #2}\par\smallskip{\scriptsize\sffamily #3}%
    \end{minipage}%
  ...
  \begin{minipage}[t]{0.48\linewidth}%
    ...
    \end{minipage}\hfill\begin{minipage}[c]{0.71\linewidth}%
      {\sffamily\bfseries\small #5}\par\smallskip{\scriptsize\sffamily #6}%
    \end{minipage}%
  \end{minipage}%
  \par\medskip}
...
% \illustratedpartqr{<title>}{<image>}{<qr image>}{<qr label>}{<qr url>}
% Same as \illustratedpart but also drops a small "Watch the videos
% for this part" QR below the illustration. Saves one page per part
% compared to a standalone trailer.
\newcommand{\illustratedpartqr}[5]{%
  \clearpage
  \refstepcounter{part}%
  \addcontentsline{toc}{part}{#1}%
  \phantomsection\pdfbookmark[0]{#1}{part.#1}%
  ...
  \begin{center}
    ...
    \vspace{1.2em}
    \includegraphics[width=\linewidth,
      height=0.55\textheight,keepaspectratio]{#2}\par
    \vspace{1.4em}
    \begin{minipage}{0.85\linewidth}
      \centering
      {\small\itshape Watch the videos for this part\par}
      \vspace{0.4em}
      \includegraphics[width=0.18\linewidth]{#3}\par
      \vspace{0.3em}
      {\footnotesize #4\par}
      {\scriptsize\ttfamily #5\par}
    \end{minipage}
  \end{center}
  \vfill
  \clearpage
...
def main() -> int:
    ...
    for part_dir in sorted(parts_map.keys()):
        ...
        if illus.exists():
            ...
            part_lines: list[str] = [
                ...
                f"{{\\linkicon\\iconsep Videos: {tex_escape(part_label)}}}"
                f"{{{tex_escape(qr_url)}}}\n"
            ]
        else:
        ...
        if not illus.exists():
            ...
            part_lines.append(
                ...
                + "}{\\linkicon\\iconsep Videos: " + tex_escape(part_label) + "}{"
                + tex_escape(qr_url) + "}"
            )
    ...
    errata_qr_callout = (
        ...
        + "}{\\linkicon\\iconsep Errata for this edition (" + tex_escape(edition_label) + ")}{"
        + tex_escape(errata_qr_url) + "}"
    )
    report_qr_callout = (
        "\\qrcallout{" + latex_path(report_qr_path)
        + "}{\\linkicon\\iconsep Report a problem (email)}{"
    ...
    sample_qr_callout = (
        ...
        + "}{\\linkicon\\iconsep Try it: open vimfubook.com}{}"
    )
