...
- **Spell out key names** in narration for TTS clarity. **Preferred:** use the `{key:...}` markup in your `say` strings — `{key:zn}` → "zee en", `{key:zN}` → "zee capital en", `{key:Ctrl-W}` → "control double-you", `{key:Esc}` → "escape". The TTS layer expands the markup to NATO-style spelling automatically, so `{key:zi}` won't be mispronounced as "zai". Use the same exact tokens as the on-screen keystrokes — that way the JSON source stays grep-able and the spoken cadence matches what the viewer sees.
